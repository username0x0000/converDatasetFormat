...
class Image:
    ...
    def get_contour(self, img_name=None):
        if img_name:
    ...
    def make_seperate_mask(self, idx=None):
        if idx:
            ...
            for img_name in self.img_list:
                ...
                for i in range(len(img)):
                    for j in range(len(img[0])):
                        ...
                        mask_img[object_num][i, j] = 255
                for key in mask_img.keys():
                    cv2.imwrite(os.path.join(self.save_path, f'{img_name.split(".")[0]}_{key}.jpg'), mask_img[key])


def main():
    img_path = './mask_data'
    image = Image(img_path, './label_data')
    # image.make_seperate_mask()
    image.get_contour()
